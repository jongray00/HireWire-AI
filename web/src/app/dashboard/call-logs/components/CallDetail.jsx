...
export default function CallDetail({ log }) {
  ...
  const tabs = [
    ...
    { id: "functions", label: `Functions (${raw.swaig_log?.length || 0})` },
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mt-3">
      ...
      {/* Role distribution + latency overview side by side */}
      ...
      {tab === "transcript" && <TranscriptPanel callLog={raw.call_log} currentPlaybackTime={playbackTime} />}
      {tab === "charts" && (
      ...
      {tab === "functions" && <SwaigPanel swaigLog={raw.swaig_log} />}
      {tab === "summary" && <SummaryPanel log={log} />}
      {tab === "postprompt" && <PostPromptTabs postPromptData={raw.post_prompt_data} />}
      {tab === "timeline" && <CallTimelineSwimlane callLog={raw.call_log} callStartDate={raw.call_start_date} />}
      {tab === "state" && (
